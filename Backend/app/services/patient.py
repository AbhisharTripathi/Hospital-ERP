...
class PatientServices:
    ...
    async def get_all_patients(self, limit):
        return await self.patient_repo.get_all_patients(limit)

    async def search_patients(self, search: PatientSearch):
        filters = {}
        ...
        return await self.patient_repo.search_patients(filters)

    async def update_patient(self, patient_id: str, patient_update: PatientUpdate):
        patient = await self.get_patient_by_id(patient_id) # Using corrected method name

        if not patient:
            raise ValueError("Patient not found")

        update_data = patient_update.model_dump(exclude_unset=True,mode="json")
        update_data["updated_at"] = datetime.now(timezone.utc)

        result = await self.patient_repo.update_by_patient_id(patient_id, update_data)

        return {"modified_count": result.modified_count}

    async def deactivate_patient(self, patient_id: str):
        patient = await self.get_patient_by_id(patient_id) # Using corrected method name
